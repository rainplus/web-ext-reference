...
`webExtension`:
    ...
    // bootstrapped add-onfunction startup({webExtension}) {...
...
WebExtension object using the `sdk/webextension` module:
    ...
    // SDK add-onconst webExtension = require("sdk/webextension");
...
For example:
    ...
    // bootstrapped add-onfunction startup({webExtension}) {  webExtension.startup().then(api => {    const {browser} = api;    browser.runtime.onMessage.addListener(handleMessage);  });}



    
    
    // SDK add-onconst webExtension = require("sdk/webextension");webExtension.startup().then(api => {  const {browser} = api;  browser.runtime.onMessage.addListener(handleMessage);});
...
message to a content script\) tabs.sendMessage\(\).") object:
    ...
    // bootstrapped add-onfunction startup({webExtension}) {  // Start the embedded webextension.  webExtension.startup().then(api => {    const {browser} = api;    browser.runtime.onMessage.addListener((msg, sender, sendReply) => {      if (msg == "message-from-webextension") {        sendReply({          content: "reply from legacy add-on"        });      }    });  });}

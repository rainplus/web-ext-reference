...
### Creating the store and saving the images
    ...
    async function saveCollectedBlobs(collectionName, collectedBlobs) { const storedImages = await getFileStorage({name: "stored-images"}); for (const item of collectedBlobs) {    await storedImages.put(`${collectionName}/${item.uuid}`, item.blob); }}
...
### Retrieving stored images for display
    ...
    export async function loadStoredImages(filter) {\xa0const imagesStore = await getFileStorage({name: "stored-images"});\xa0let listOptions = filter ? {includes: filter} : undefined;\xa0const imagesList = await imagesStore.list(listOptions);\xa0let storedImages = [];\xa0for (const storedName of imagesList) {\xa0\xa0\xa0 const blob = await imagesStore.get(storedName);\xa0\xa0\xa0 storedImages.push({storedName, blobUrl: URL.createObjectURL(blob)});\xa0}\xa0return storedImages;}
...
### Delete collected images
    ...
    async function removeStoredImages(storedImages) {\xa0const imagesStore = await getFileStorage({name: "stored-images"});\xa0for (const storedImage of storedImages) {\xa0\xa0\xa0 URL.revokeObjectURL(storedImage.blobUrl);\xa0\xa0\xa0 await imagesStore.remove(storedImage.storedName);\xa0}}
